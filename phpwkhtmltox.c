...
static zend_function_entry phpwkhtmltox_functions[] = {
    ...
    PHP_FE(wkhtmltox_convert_image, NULL)
    PHP_FE(wkhtmltox_convert_pdf, NULL)
    {NULL, NULL, NULL}
};
...
PHP_FUNCTION(wkhtmltox_convert_image)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sa|z",
            &input, &input_len, &global_params, &errorlog) == FAILURE) {
        RETURN_NULL();
    }
    ...
    // we dont use the 'in' key, but wkhtml expects it as '-' if using manual
    wkhtmltoimage_set_global_setting(global_settings, "in", "-");

    wkhtmltoimage_converter *c = wkhtmltoimage_create_converter(global_settings, input);

    MAKE_STD_ZVAL(wkhtmltox_errorlog);
    array_init(wkhtmltox_errorlog);
    wkhtmltoimage_set_warning_callback(c, (wkhtmltopdf_str_callback)wkhtmltox_warning);
    wkhtmltoimage_set_error_callback(c, (wkhtmltopdf_str_callback)wkhtmltox_error);

    ret = wkhtmltoimage_convert(c);
    output_len = wkhtmltoimage_get_output(c, &output);
...
PHP_FUNCTION(wkhtmltox_convert_pdf)
{
    ...
    wkhtmltopdf_global_settings *global_settings = wkhtmltopdf_create_global_settings();
    wkhtmltox_set_object_params((void *)global_settings, (fp)wkhtmltopdf_set_global_setting, global_params, input);
    ...
    for(zend_hash_internal_pointer_reset_ex(params_hash, &pointer);
        ...
        if (Z_TYPE(temp) == IS_ARRAY) {
            ...
            switch (wkhtmltox_set_object_params((void *)object_settings, (fp)wkhtmltopdf_set_object_setting, &temp, input)) {
                case 0:
                    wkhtmltopdf_add_object(c, object_settings, NULL);
                    object_added = 1;
                    break;
